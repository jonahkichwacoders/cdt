 ...
 * the style pars that are global for a terminal are here, where as in TerminalStyle is about
 * a specific range.
 */
public class StyleMap {
	...
	public ColorDescriptor getForegrondColorDescriptor(TerminalStyle style) {
		...
		if (foregroundRGB != null) {
			return ColorDescriptor.createFrom(foregroundRGB);
		}
		...
		return color.getColorDescriptor(fInvertColors, style.isBold());
	}

	public ColorDescriptor getBackgroundColorDescriptor(TerminalStyle style) {
		style = defaultIfNull(style);
		RGB backgroundRGB;
		if (style.isReverse()) {
		...
		if (backgroundRGB != null) {
			return ColorDescriptor.createFrom(backgroundRGB);
		}
